...
while IFS= read -r -d '' file; do
    . "\$file"
done < <(find -L "$escaped_conf_dir" -type f -print0 | sort -z)
###
...
EOF
    ...
    if [ -d "$source_conf_dir" ]; then
        while IFS= read -r -d '' file; do
            ...
            if [ ! -e "$link_path" ]; then
                print_and_run ln --symbolic --relative "$link_target_path" "$link_path"
            fi
